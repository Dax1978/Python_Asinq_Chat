...
class Server(threading.Thread, metaclass=ServerMaker):
    ...
    def arg_parser(self):
        ...
        parser.add_argument('-a', default='', type=str, nargs='?', help='Параметр -a позволяет указать IP-адрес, с которого будут приниматься соединения (по умолчанию адрес не указан, сервер будет принимать со всех адресов)')
    ...
    def get_addr_port(self):
        ...
        #         f' подключения: {self.ADDR}. Если адрес не указан, принимаются соединения с любых адресов.')
        print(f'Запущен сервер...\nПорт для подключений: {self.PORT}')
